...
def _gif_search_next_previous(action, index):
    ...
    if len(data["urls"]) > data["index"] + 1:
        response["attachments"][0]["actions"].insert(
            1, {"name": "next", "text": "Next", "value": data_json, "type": "button"}
        )
...
async def recording_cancel(action, app):
    ...
    async with app["plugins"]["pg"].connection() as pg_con:
        await pg_con.execute(
            """DELETE FROM slack.recordings WHERE id = $1""", recording_id
        )
...
async def recording_message(action, app):
    ...
    async with app["plugins"]["pg"].connection() as pg_con:
        result = await pg_con.fetchval(
            ...
            ), commit = TRUE WHERE id = $2 RETURNING id""",
            action["message_ts"],
            recording_id,
        )
...
async def recording_emoji(action, app):
    ...
    if created:
        ...
        async with app["plugins"]["pg"].connection() as pg_con:
            exist = await pg_con.fetchval(
                """SELECT id FROM slack.recordings WHERE id = $1""", recording_id
            )
...
async def save_conversation(action, app):
    ...
    if end_delay >= start_delay:
        errors = [
            {"name": "start", "error": "Start time must be superior than end time"},
            {"name": "end", "error": "End time must be inferior than start time"},
        ]
