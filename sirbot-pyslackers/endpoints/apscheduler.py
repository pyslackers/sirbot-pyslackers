...
def create_jobs(scheduler, bot):
    ...
    scheduler.scheduler.add_job(slack_users_list, 'date', kwargs={'bot': bot},
...
async def slack_users_list(bot):
    ...
    async with bot['plugins']['pg'].connection() as pg_con:
        async for user in bot['plugins']['slack'].api.iter(methods.USERS_LIST, minimum_time=12):
